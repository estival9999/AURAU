"""
Teste de Performance do Sistema RAG com Busca Híbrida (Versão Headless)
Analisa diferentes configurações e salva resultados
"""
...
class TestadorPerformanceRAG:
    ...
    def analisar_resultados(self):
        ...
        print("\n📊 Melhor configuração por tipo de query:")
        for tipo in df['tipo_query'].unique():
            df_tipo = df[df['tipo_query'] == tipo]
            melhor = df_tipo.groupby('config_nome')['score_medio'].mean().idxmax()
            score = df_tipo.groupby('config_nome')['score_medio'].mean().max()
            print(f"   {tipo}: {melhor} (score: {score:.4f})")
        ...
        resumo = {
            "melhor_geral": melhor_config.index[0],
            "score_melhor": float(melhor_config.iloc[0]['score_medio']),
            "mais_rapida": melhor_config.sort_values('tempo_total').index[0],
            "tempo_mais_rapida": float(config_rapida['tempo_total']),
            "melhor_equilibrio": df_balance.sort_values('eficiencia', ascending=False).index[0],
            "eficiencia_melhor": float(melhor_balance['eficiencia'])
        }
        
        with open(f"resumo_performance_{timestamp}.json", 'w') as f:
            json.dump(resumo, f, indent=2)
        
        print(f"📄 Resumo salvo em: resumo_performance_{timestamp}.json")
...
def main():
    ...
    print("\n📁 Arquivos gerados:")
    print("   • Gráfico de análise: analise_performance_rag_*.png")
    print("   • Dados detalhados: resultados_detalhados_*.csv")
    print("   • Resumo: resumo_performance_*.json")
